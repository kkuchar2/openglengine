...
class BoundingBoxGenerator {
    public:
        ...
        static void calculateBoundingBox(const std::shared_ptr<Mesh> & mesh, const std::shared_ptr<GameObjectBase> & child) {
            ...
            cubeMeshProto->color = glm::vec4(0.0, 1.0, 0.0f, 0.2f);

            std::shared_ptr<GameObjectBase> bboxObject = std::make_shared<GameObjectBase>();

            bboxObject->addComponent(cubeMeshProto);

            child->bbox.center = b.center;
            child->bbox.size = b.size;
            child->boundingBox = bboxObject;
        }
};